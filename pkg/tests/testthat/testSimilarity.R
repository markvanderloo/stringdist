...
for (method in c("osa", "lv", "dl", "hamming", "lcs", "qgram", "cosine", 
  ...
  test_that(paste0("Similarity is between 0 and 1 for ", method), {
    expect_that(similarity("bb", "cc", method=method), equals(0))
    expect_that(similarity("bb", "bb", method=method), equals(1))
  })

  test_that(paste0("Edge cases for ", method), {
    expect_that(similarity(c("a", ""), "", method=method, q=0), equals(c(0, 1)))
  })
}
